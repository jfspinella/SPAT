...
SPAT (Surface Protein Annotation Tool) Version 3 - updated april-2021
...
if(!defined $best_score){
    print "\n- By default, if several nextprot references exist for a protein, only the entry with the best SPAT score will be reporter. Use the option \"-best_score false\" to output all entries.\n";
    $best_score = "true";
}elsif($best_score eq "false"){
    print "\n- The option \"-best_score\" has been turned off. All entries will be reported (several for a protein if several nextprot references exist). The SPAT score may differ between entries according to associated annotations.\n";
}else{
    print "\n- The option \"-best_score\" has been turned on. If several nextprot references exist for a protein, only the entry with the best SPAT score will be reporter. Use the option \"-best_score false\" to output all entries (default value is true).\n";
    $best_score = "true";
}

if(!defined $score_threshold){
    print "\n- No score threshold has been set (default threshold is >=1).\n";
    $score_threshold = 1;
...
my $file2 = "./database/nextprot_all.ref.txt";
my %ref;
...
### Create hash with nextprot_all.term info

my $file3 = "./database/nextprot_all.term.txt";
my %spat;
...
while (my $l =<FILE3>){
    ...
    if (exists $l[1]){
        ...
        ## CREATE HASH ##
        $spat{$l[0]} = $l[0]."\t$membrane_score\t$membrane_class\t$S1_count,$S2_count,$S3_count,$S4_count,$S5_count\t".join( ',', @membrane_terms )."\t".join( ',', @annotation )."\t".join( ',', @ref )."\t$RF_count\t".join( ',', @rf_terms )."\t$SF_count\t".join( ',', @sf_terms )."\t".$l[2];
    ...
    if(exists $spat{$l[0]}){
        if($format eq "uniprot_id"){
            ...
            if(exists $transl{$l[1]}){
                # test for redundancy in nextprot ids
...
# human prot atlas
my $file5 = "./database/prot_atlas.txt";
my %atlas;
...
print (OUT "INPUT\tGENE_NAME\tNEXTPROT_ID\tSPAT_SCORE_v3\tSURF_CLASS\tSURF_CLASS_COUNTS\tUSED_SURF_TERMS\tSURF_TERMS_ANNOTATION\tPUBMED_IDs\tRED_FLAG_COUNT\tRED_FLAG_TERMS\tSECRETED_FLAG_COUNT\tSECRETED_FLAG_TERMS\tMAIN_LOCATION_NEXTPROT\tMAIN_LOCATION_HPA\tAPPROVED_ANTIBODY(see https://antibodyregistry.org/)\tSIGNIFICANT_PROGNOSTIC_VALUE_HPA\tTISSUE_RNA_HPA(NX)\tTISSUE_PROT_HPA(level)\tHEMOGENE_RNA(TPM)\n");

my $output2 = $output_path.$file_name.".log.out";
open (OUT2, "> $output2") or die "Can't open $output2: $!";
print (OUT2 "### COMMAND LINE (runned $datestring) ###\n\nperl SPAT.v3.pl -input $input -format $format -output $output_path\n\n");
print (OUT2 "\n### UNANNOTATED PROTEINS (MISSING IN THE DATABASE) ###\n\n");
    ...
    chomp $l;
        ...
        if(exists $transl{$l}){
            ...
            if($best_score eq "true"){
                ...
                foreach my $nextprot(@{$transl{$l}}){
                    my @l = split(/\t/, $spat{$nextprot});
                    push @scores, $l[1];
                }
                ...
                $nextprot = ${$transl{$l}}[$idx];
                
                if(exists $spat{$nextprot}){
                        
                    $out{$nextprot}=$l."\t".$gene_name{$nextprot}."\t".$spat{$nextprot};
                        
                    if(exists $atlas{$nextprot}){
                        $out{$nextprot}=$out{$nextprot}."\t".$atlas{$nextprot};
                    }else{
                        $out{$nextprot}=$out{$nextprot}."\tNA\tNA\tNA\tNA\tNA";
                    }
                        
                    if(exists $hemogene{$nextprot}){
                        $out{$nextprot}=$out{$nextprot}."\t".$hemogene{$nextprot};
                    }else{
                        $out{$nextprot}=$out{$nextprot}."\tNA";
                    }
...
print (OUT2 "\n\n### SURFACE PROTEIN ANNOTATION ###\n\n-To create your distribution plot, run:\n\tRscript $output_path$file_name.SPAT_distribution.R\n\n-Summary:\n\t$i/$tot ($perc_anno%) annotated proteins (see $file_name.SPAT_annotation.txt)\n\t$j/$tot ($perc_missing%) unannotated proteins (see list above)\n");
...
print (OUT3 'suppressMessages(library(ggplot2))'."\n".'surf = read.table("'.$output.'", sep="\t", header = TRUE)'."\n".'g = ggplot(surf, aes(SPAT_SCORE_v3)) + geom_density(adjust=1.2)'."\n".'y_max = ggplot_build(g)$layout$panel_scales_y[[1]]$range$range[2]'."\n".'png("'.$output_path.$file_name.'.SPAT_distribution.png", width=500, height=500, res=200)'."\n".'ggplot(surf, aes(SPAT_SCORE_v3)) + geom_density(color="black", lwd=1, fill="grey", alpha=0.6, adjust=1.2) + geom_vline(aes(xintercept=mean(SPAT_SCORE_v3)), color="#69b3a2", linetype="dashed", size=.7) + labs(x = "SPAT score") + scale_color_hue(name="Type") + theme_classic() + theme(plot.title = element_text(size=10)) + labs(title="SCORE DISTRIBUTION") + geom_label(label=paste("mean=",format(round(mean(surf$SPAT_SCORE_v3), 2), nsmall = 2),sep=""), x = mean(surf$SPAT_SCORE_v3), y = y_max, size = 1.8, label.padding = unit(0.2, "lines"), color = "black", fill="#69b3a2")'."\n".'dev.off()'."\n");  
close OUT3;
